...
**`setupDetailScroll` — full function definition (already in `main.js`):**

This function is defined in `main.js` and must not be duplicated in page `<script>` tags. It watches the scroll position and updates the sticky time/location panel with a smooth fade+slide transition as each `.detail-entry` crosses 38% from the top of the viewport.
...
// Init — one line per day, at the bottom of main.js:
setupDetailScroll('d1-entries','d1-time','d1-ampm','d1-loc');
setupDetailScroll('d2-entries','d2-time','d2-ampm','d2-loc');
```

**How it works:**
- On every scroll event it finds the last `.detail-entry` whose top edge is within 38% + 80px of the viewport top — that's the entry currently being read.
- It marks it `is-active` and fades the sticky panel values out, swaps the text, then fades back in.
- Runs once on load so the panel is populated before any scrolling.

**Wire up each new day at the bottom of `main.js`** — one line per day:
```js
// Add for Day 3 if needed:
setupDetailScroll('d3-entries','d3-time','d3-ampm','d3-loc');
```

**What NOT to do:**
- Never call `setupDetailScroll` in an inline `<script>` tag — it is already called in `main.js`. Calling it twice registers the scroll listener twice, causing double updates.
- Do not omit `data-time`, `data-ampm`, or `data-loc` from any `.detail-entry` — the panel will go blank.
- Do not rename the `.detail-entry` class — the function queries by it.
